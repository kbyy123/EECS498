...
def prepocess_input_sequence(
    ...
    for ele in input_str_split:
        if ele in spc_tokens:
            out.append(token_dict[ele])
        else:
            for token in ele:
...
def scaled_dot_product_two_loop_single(
    ...
    K, M = query.shape
    sqrt_M = math.sqrt(M)
    scores = query.new_zeros((K, K))
    for i in range(K):
        for j in range(K):
            scores[i, j] = query[i].vdot(key[j]) / sqrt_M
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    scores = query.new_zeros((N, K, K))
    sqrt_M = math.sqrt(M)
    for i in range(N):
        scores[i] = (query[i] @ key[i].t() / sqrt_M).softmax(dim=1)
    out = scores @ value
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    weights_softmax = query @ key.transpose(1, 2) / math.sqrt(M)
    if mask is not None:
        weights_softmax = weights_softmax.masked_fill_(mask, -1e9)
    # Replace "pass" statement with your code
    weights_softmax = weights_softmax.softmax(dim=-1)
...
#         self.q = nn.ModuleList(nn.Linear(dim_in, dim_out) for _ in range(num_heads))
#         self.k = nn.ModuleList(nn.Linear(dim_in, dim_out) for _ in range(num_heads))
#         self.v = nn.ModuleList(nn.Linear(dim_in, dim_out) for _ in range(num_heads))
#         self.projection_layer = nn.Linear(num_heads * dim_out, dim_in)

#         c = math.sqrt(6 / (dim_in + dim_out))
#         for i in range(num_heads):
#             nn.init.uniform_(self.q[i].weight, -c, c)
#             nn.init.uniform_(self.k[i].weight, -c, c)
#             nn.init.uniform_(self.v[i].weight, -c, c)
#         nn.init.uniform_(self.projection_layer.weight, -c, c)
#         ##########################################################################
#         #               END OF YOUR CODE                                         #
#         ##########################################################################
...
#             k = self.k[i](key)
#             v = self.v[i](value)
#             y, _ = scaled_dot_product_no_loop_batch(q, k, v, mask)
#             out.append(y)
        
#         y = self.projection_layer(torch.cat(out, dim=-1))
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.q = nn.ModuleList(
            nn.Linear(dim_in, dim_out) for _ in range(num_heads)
        )
    ...
    def forward(
        ...
        for i in range(self.num_heads):
            ...
            head_out, weights_softmax = scaled_dot_product_no_loop_batch(
                q, k, v, mask
            )

            out.append(head_out)
            attention_weights.append(weights_softmax.detach())

        # shape: (N, num_heads, K, K)
        self.weights_softmax = torch.stack(attention_weights, dim=1)
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        nn.init.uniform_(self.linear_1.weight, -c, c)
        nn.init.uniform_(self.linear_2.weight, -c, c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(self, x):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        y = self.linear_2(torch.relu(self.linear_1(x)))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.dropout(self.layer_norm_2(out3 + out2))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    div_term = torch.exp(torch.arange(0, M, 2) / M * (-math.log(10000)))
    
    y = torch.zeros(K, M)
    y[:, ::2] = torch.sin(position * div_term)
    if M % 2 == 0:
        y[:, 1::2] = torch.cos(position * div_term)
    else:
        y[:, 1::2] = torch.cos(position * div_term[:-1])
    y = y.unsqueeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        q_emb_inp = q_emb + ques_pos # (N, K, M)
        ...
        dec_out = self.decoder(a_emb_inp, enc_out, mask).flatten(0, 1)
