...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flat = x_train.reshape(num_train, -1)
    x_test_flat = x_test.reshape(num_test, -1)
    x_train_square_sum = torch.sum(x_train_flat**2, dim=1)
    x_test_square_sum = torch.sum(x_test_flat**2, dim=1)
    cross = torch.matmul(x_train_flat, x_test_flat.t())
    dists = x_train_square_sum.reshape(-1, 1) + x_test_square_sum.reshape(1, -1) - 2 * cross
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    top_k_index = torch.topk(dists, k, dim=0, largest=False).indices
    closest_labels = y_train[top_k_index]
    for i in range(num_test):
        y_pred[i] = torch.argmax(torch.bincount(closest_labels[:, i]))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    accuracy = torch.zeros((len(k_choices), num_folds))
    for i in range(num_folds):
        x_train_folds_except_i = torch.cat(x_train_folds[:i] + x_train_folds[i + 1:], dim=0)
        y_train_folds_except_i = torch.cat(y_train_folds[:i] + y_train_folds[i + 1:], dim=0)
        classifier = KnnClassifier(x_train_folds_except_i, y_train_folds_except_i)
        for j in range(len(k_choices)):
            accuracy[j, i] = classifier.check_accuracy(x_train_folds[i], y_train_folds[i], k_choices[j])
    for i in range(len(k_choices)):
        k_to_accuracies[k_choices[i]] = accuracy[i].tolist()
